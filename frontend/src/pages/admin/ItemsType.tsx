...
import EditItemsTypeModal from './modals/EditItemsType';
...
function ItemsType() {
  ...
  const [selectedUser, setSelectedUser] = useState<ItemsType | null>(null);
  ...
  const filteredUsers = users.filter(user => {
    const matchesSearch = user.items_type.toLowerCase().includes(searchTerm.toLowerCase());
    return matchesSearch;
  });

  const handleEdit=(user: ItemsType)=>{
    setSelectedUser(user);
    setEditModelOpen(true);
  }

  const handleDelete = (user: ItemsType) => {
    setSelectedUser(user);
    setDeleteModalOpen(true);
  };
  
  const handleSave = async (updatedItemsType: ItemsType) => {
    // Example PUT API call
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`http://localhost:8080/api/v1/items-type/${updatedItemsType._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(updatedItemsType),
      });

      if (response.ok) {
        toast.success('Items Type Updated Successfully!');
        // Update local state or re-fetch list
        setUsers(prev =>
          prev.map(user =>
            user._id === updatedItemsType._id ? updatedItemsType : user
          )
        );
        setEditModelOpen(false);
      } else {
        console.error('Failed to update Items type');
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const confirmDelete = async () => {
    if (!selectedUser?._id) return;
      try {
         const token = localStorage.getItem('authToken');
        const response = await fetch(`http://localhost:8080/api/v1/items-type/${selectedUser._id}`, {
          method: 'DELETE',
            headers: {
           'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        if (response.ok) {
          toast.success('Items Type deleted successfully!');
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-white">Items Type Management</h1>
        <button  onClick={() => navigate('/admin/items-type/add')} className="flex items-center gap-2 bg-primary hover:bg-primary-dark text-white px-4 py-2 rounded-md transition-colors duration-200 text-sm">
          <PlusCircle size={18} />
          <span>Add New Items Type</span>
        </button>
      </div>
      ...
      <DeleteConfirmationModal
        ...
        title="Delete User"
        message={`Are you sure you want to delete ${selectedUser?.items_type}? This action cannot be undone.`}
      />
